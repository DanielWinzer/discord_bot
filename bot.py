...
def log(user, server, user_command):
	...
	f.write(
	    str(user) + " | " + str(server) + " | " + str(user_command) + " | " +
	    str(log_time) + "\n")
	f.close()
	...
	with open("log/hash_bot_user_log.txt", "a") as f:
		f.write(
		    str(hash_text(user)) + " | " + str(hash_text(server)) + " | " +
		    str(user_command) + " | " + str(log_time) + "\n")
	with open("log/hash_bot_command_log.txt", "a") as f:
		f.write(
		    str(hash_text(server)) + " | " + str(user_command) + " | " +
		    str(log_time))
...
async def pre_stock(ctx, *, stock_date):
	...
	if int(stock_date[1]) > date.today().month and int(
	    stock_date[0]) == date.today().year:
		send = False
	if int(stock_date[2]) > date.today().day and int(
	    stock_date[1]) == date.today().month and int(
	        stock_date[0]) == date.today():
		send = False
	if send:
		picture_path = stonks.predefinded_stocks(stock_date[0] + "-" +
		                                         stock_date[1] + "-" +
		                                         stock_date[2])
...
async def iss(ctx):
	...
	embed_iss = discord.Embed(colour=discord.Colour.orange())
	embed_iss.add_field(name="Name:", value=name, inline=False)
	embed_iss.add_field(name="Höhe:", value=f'{altitude} km', inline=False)
	embed_iss.add_field(name="Geschwindigkeit:",
	                    value=f'{velocity} km/h',
	                    inline=False)
	await ctx.send(embed=embed_iss)
...
async def help(ctx):
	...
	embed.add_field(name=prefix_command + 'ping',
	                value='Returns ping',
	                inline=False)
	embed.add_field(name=prefix_command + 'pre_stock',
	                value='Needs a date(yyyy-mm-dd)',
	                inline=False)
	embed.add_field(
	    name=prefix_command + 'stock',
	    value=
	    'Needs a stock("gme") and period("1d,5d,1mo,3mo,6mo,1y,2y,5y,10y")',
	    inline=False)
	embed.add_field(name=prefix_command + "shoot",
	                value="Russain Roulette",
	                inline=False)
	embed.add_field(name=prefix_command + "say", value="tts", inline=False)
	embed.add_field(name=prefix_command + "join",
	                value="joins your voice channel",
	                inline=False)
	embed.add_field(name=prefix_command + "leave",
	                value="leaves your voice channel",
	                inline=False)
	embed.add_field(name=prefix_command + "queue <url>",
	                value="add a song to the queue",
	                inline=False)
	embed.add_field(name=prefix_command + "remove <index>",
	                value="removes a song on index",
	                inline=False)
	embed.add_field(name=prefix_command + "play",
	                value="plays the queue",
	                inline=False)
	embed.add_field(name=prefix_command + "pause",
	                value="pauses current song",
	                inline=False)
	embed.add_field(name=prefix_command + "resume",
	                value="resumes paused song",
	                inline=False)
	embed.add_field(name=prefix_command + "view",
	                value="shows queue",
	                inline=False)
	embed.add_field(name=prefix_command + "stop",
